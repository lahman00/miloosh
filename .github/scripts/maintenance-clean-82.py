...
# Repair stale affiliate-program evidence without inventing unknown terms.
...
# Replace Keeper's obsolete generic pricing URL with the current first-party pages.
...
# GitHub Actions intentionally does not carry Vercel production social secrets.
# Treat missing channel env there as unverifiable rather than production-broken.
...
# A timeout/connection failure proves only that this runner could not reach a host.
# Reserve critical severity for definitive broken references or malformed URLs.
p = Path("scripts/maintenance/links.ts")
text = p.read_text()
old = 'const CRITICAL_OUTCOMES: LinkCheckOutcome[] = ["not_found", "gone", "invalid_url", "connection_failure", "timeout"];\n\nfunction severityForOutcome(outcome: LinkCheckOutcome): "critical" | "warning" {'
new = 'const CRITICAL_OUTCOMES: LinkCheckOutcome[] = ["not_found", "gone", "invalid_url"];\n\nexport function severityForOutcome(outcome: LinkCheckOutcome): "critical" | "warning" {'
if old not in text:
    raise SystemExit("link severity anchor missing")
text = text.replace(old, new, 1)
p.write_text(text)

p = Path("tests/lib/link-severity.test.ts")
if p.exists():
    raise SystemExit("link severity regression test already exists")
